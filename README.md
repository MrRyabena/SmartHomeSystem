...
- [6. Этап I](#6_stageI)
  ...
  - [6.2 Управление нагрузкой](#6-2_load)
    - [6.2.1 Силовые схемы](#6-2-1_power-circuits)
    - [6.2.2 Режим ```SWITCH```](#6-2-2_modeSWITCH)
    - [6.2.3 Режим ```PWM```](#6-2-3_modePWM)
    - [6.2.4 Режим ```DIMMER```](#6-2-4_modeDIMMER)
    - [6.2.5 Режим ```PID```](#6-2-5_modePID)
  - [6.3 Датчики](#6-3_sensors)
    - [6.3.1 Кнопка, энкодер, потенциометр](#6-3-1_button)
    - [6.3.2 Термистор и фоторезистор](#6-3-2_thermistor)
    - [6.3.3 Термопара](#6-3-3_thermocouple)
    - [6.3.4 DS18B20](#6-3-4_ds18b20)
    - [6.3.5 HTU21D](#6-3-5_htu21d)
...
[SHScore](src/SHScore/) — логика работы системы, основанная на парадигме **ООП** и принципах **SOLID**. В ядре описаны абстрактные классы для оперируемых системой объектов и процессов, а так же обработчики для них, протоколы связи и передачи данных. Ядро дискретизирует и описывает действия системе, что позволяет быстро и удобно создавать и изменять конфигурацию системы и последующие классы высокого уровня (библиотеки для конкретных задач).</br>
...
_**Smart Home System File**_ — протокол записи данных в файл, а так же само расширение фала (.shsf). Содержит инструменты для записи, сжатия, чтения данных, сохранения настроек и конфигураций.</br>
...
- ```SWITCH``` — нагрузку можно только полностью включить или полнотью выключить.
- ```PWM``` — можно управлять мощностью нагрузкив сети постоянного тока с помошью ШИМ.
- ```DIMMER``` — аналог ШИМ для сети перенного тока.
...
Вторым шагом необходимо научить все устройства находить друг друга в сети. Для этого им надо раздать статические IP-адреса. Такой адрес привязывается к MAC-адресу платы (он идет с завода, но при желании можно поменять). Делается все в настройках роутера, процедура не сложная. Теперь, зная IP устройства, можно будет посылать и принимать данные.</br>
