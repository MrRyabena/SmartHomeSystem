...
Сначала необходимо создать объект класса `shs::DTP`. Он отвечает за связывание всех шин данных и обработку приходящих по ним сообщений. Чтобы принимать и обрабатывать входящие подключения, необходимо запустить TCP Server. Он реализован в классе `shs::TcpServer`. Серверу нужно передать ссылку на объект `shs::DTP`, таким образом, при подключении нового клиента сервер будет создавать "шину данных" и добавлять ее в хранилище DTP.
...
В коде создадим объект термистора, передав в него характеристики используемого компонента, которые мы ранее посмотрели в даташите. Сразу подключим к нему API, чтобы другие устройства могли запрашивать и получать информацию.
