...
> [!IMPORTANT]
>
> Для запуска исходных файлов [src/examples/guide_system/](../../src/examples/guide_system/) необходимо запустить файл [make.sh](../../src/examples/guide_system/make.sh), находящийся в дирректории с исходниками. Данный скрипт аналогичен описанному в [build_manual.md](build_manual.md).
...
Создадим файл [gs_config.h](../../src/examples/guide_system/gs_config.h), в котором укажем конфигурацию всех трех модулей в системе. Теперь, при его подключении в код нам будут доступны следующие константы:
...
Из этих констант составляются `shs::ID`, которые используются в API. Например, для работы с нагрузкой `shs::ID` будет выглядеть так:
...
> [!NOTE]
> Если имя сети и пароль указаны в файле настроек (`shs_settings.h`), то передавать аргументы в функцию необязательно. Подробнее см. [документация к ControlWiFi](../Code/SHScore_doc/shs_doc_ControlWiFi.md).
...
> [!NOTE]
>
> Чтобы не загромождать код, был убран вывод отладочной информации._
...
Добавим к прошивке Module_1_sensor следующий код:
...
В результате выполнения всех этапов, описанных в данном гайде, нам удалось создать функционирующую модульную систему на базе фреймворка **SmartHomeSystem**. Каждый модуль — будь то датчик, исполнительное устройство или графический интерфейс — был реализован как самостоятельный элемент, взаимодействующий с другими через унифицированные протоколы и API.

Главное преимущество использования **SmartHomeSystem** заключается в том, что платформа предоставляет готовые инструменты для организации связи между модулями, управления устройствами и обмена данными. Это позволило существенно сэкономить время и силы, которые обычно тратятся на разработку низкоуровневых протоколов, обработку сетевых соединений и реализацию коммуникации между компонентами. Вместо написания большого объёма кода для обеспечения взаимодействия, мы смогли сосредоточиться на логике работы системы и её функциональности.

**SmartHomeSystem** обладает рядом важных достоинств:
- **Модульность и масштабируемость** — легко добавлять новые устройства и расширять систему без необходимости переписывать существующий код.
- **Гибкость** — поддержка различных типов микроконтроллеров, датчиков и исполнительных устройств.
- **Унифицированные интерфейсы** — стандартизированные API для работы с устройствами и обмена данными.
- **Простота интеграции** — наличие вспомогательных библиотек для быстрой интеграции с популярными платформами (например, Qt для создания GUI).
- **Открытость и расширяемость** — проект открыт для доработок и может быть адаптирован под специфические задачи пользователя.

Таким образом, **SmartHomeSystem** позволяет быстро и удобно реализовывать проекты умного дома и других систем автоматизации, минимизируя рутинную работу и снижая порог вхождения для новых пользователей.
