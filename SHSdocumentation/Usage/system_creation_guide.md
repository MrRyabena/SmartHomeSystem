...
В основе каждого модуля лежит ЭВМ — микроконтроллер или компьютер. Для первого и второго модулей мы будем использовать ESP8266. Можно использовать любой из поддерживаемых микроконтроллеров с наличием WiFi: [см. документацию по микроконтроллерам](../Hardware/microcontrollers.md).
...
В качестве `Module_3_gui` будет использоваться персональный компьютер или ноутбук. При желании приложение можно собрать под мобильные устройства.

#### Датчик

Датчик в нашей системе будет измерять показания температуры. Это, наверное, наиболее распространенный пример, который легко повторить — термодатчики дешевые и доступные. Можно взять любой другой датчик, поддерживаемый **_SmartHomeSystem_**, при этом никаких дополнительных изменений (кроме его инициализации класса, реализующего работу с интерфейсом датчика) в систему вносить не придется.
...
Следующим шагом необходимо разработать принципиальную электрическую схему. Для этого следует использовать документацию от производителей и материалы из интернета. Некоторую информацию можно найти в документации к аппаратной части проекта [см. SHSdocumentation/Hardware](../Hardware/).
