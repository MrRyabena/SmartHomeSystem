...
> - `Component` — неделимая составляющая. Под ним подразумевается класс, процесс, вычислительный блок или малое устройство (отдельный датчик, соединение, протокол...).
>
...
Первым делом следует установить необходимые для работы программы, утилиты и драйверы, а затем запустить сборку проекта. После это можно приступать к использованию **_SmartHomeSystem_**. Весь этап подробно описан в отдельном файле документации — [build_manual.md](build_manual.md)
...
В будущем, планируется реализовать более удобную автоматическую конфигурацию системы, ровно как и дальнейшее создание классов и обработчиков. В данной версии проекта много придется прописать вручную, однако это пустяки, по сравнению с "голым кодом", который бы пришлось создавать без использования **_SmartHomeSystem_**. Итак, переходим к делу...
...
Нам необходимо подключить модули к WiFi сети, чтобы можно было организовывать передачу данных. Для этого в `SHScore` есть специальный класс `shs::ControlWiFi`.
