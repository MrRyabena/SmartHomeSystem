...
Первым делом следует установить необходимые для работы программы, утилиты и драйверы. Затем нужно запустить сборку проекта, после чего можно будет приступать к использованию. Весь этап подробно описан в отдельном файле документации — [build_manual.md](build_manual.md)

### Конфигурация системы

Для начала необходимо создать конфигурацию системы, которая будет едина для всех модулей. С её помощью мы сможем удобно использовать идентификаторы модулей и устройств, избегая копирования файлов и года и предотвращая возможные ошибки и несостыковки.

В будущем, планируется более удобная автоматическая конфигурация системы, ровно как и дальнейшее создание классов и обработчиков. В данной версии проекта много придется прописать вручную, однако это пустяки, по сравнению с "голым кодом", который бы пришлось создавать без использования **_SmartHomeSystem_**. Итак, переходим к делу...
...
Теперь нам нужно сделать так, чтобы наше устройство принимало и отправляло данные (запросы с других устройств). Для этого модуль должен уметь:
...
Первым делом необходимо создать объект класса `shs::DTP`. Он отвечает за связывание воедино всех шин данных и обработку приходящих по ним сообщений. Чтобы принимать и обрабатывать входящие подключения необходимо запустить TCP Server. Он реализован в классе `shs::TcpServer`. Серверу нужно передать ссылку на объект `shs::DTP`, таким образом, при подключении нового клиента сервер будет создавать "шину данных" и добавлять ее в хранилище DTP.
...
Теперь, когда связь налажена, стоит перейти к реализации конкретно каждого модуля.

#### `Module_1`

Для считывания показаний датчика с аналогово пина предназначен класс `shs::SensorAnalog`. Сразу подключим к нему API, чтобы другие устройства могли запрашивать и получать информацию.
...
Для управления нагрузкой в режиме `SWITCH` (вкл/выкл), аналогично датчику, предоставляется класс `shs::LoadSwitch`. При необходимости плавного регулирования следует использовать `shs::LoadPWM`.
...
Тут уже все посложнее, нам предстоит написать полноценное графическое приложение. Парой строчек тут не обойдется, поэтому решению этой задачи посвящен отдельный раздел, следующий ниже.
