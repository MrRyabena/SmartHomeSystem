...
uint8_t shs::UDP::sendMulticastPacket(UDP_t& udp, const shs::DTPpacket& packet, const shs::t::shs_IP_t multicastIP, const shs::t::shs_port_t port)
{
    ...
    if (
        ...
        !udp.beginPacketMulticast(multicastIP, port, WiFi.getMode() == WIFI_STA ? WiFi.localIP() : WiFi.softAPIP())
    #elif defined(SHS_SF_ESP32)
        !udp.beginMulticast(multicastIP, port) || !udp.beginMulticastPacket()
    #endif
        ) return 0;
