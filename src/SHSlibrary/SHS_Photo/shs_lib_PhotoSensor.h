...
public:
    ...
    explicit PhotoSensor(shs::t::shs_pin_t pin, uint16_t floor_bound, uint16_t ceil_bound, uint8_t samples = SENSOR_AVERAGE_SAMPLES);
    ...
    [[nodiscard]] uint16_t getFloorBound() const { return m_floor_bound; }
    [[nodiscard]] uint16_t getCeilBound() const { return m_ceil_bound; }
